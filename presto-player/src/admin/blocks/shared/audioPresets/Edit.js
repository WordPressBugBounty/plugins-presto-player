...
function EditPlayerPreset( {
	...
	const putPreset = async () => {
		...
		try {
			...
			// eslint-disable-next-line no-console -- only error reporting on this settings-save path; removing it would silently swallow the failure.
			console.log( e );
		}
	};
	...
	const save = () => {
		if ( ! thisName ) {
			setError(
				__( 'You must enter a name for the preset.', 'presto-player' )
			);
			return;
		}
	...
	return (
		...
		>
			<div className="presto-player__preset-options" data-cy="preset-modal">
				<TextControl
					...
					onChange={ ( newName ) => setThisName( newName ) }
					placeholder={ __( 'Enter a preset name...', 'presto-player' ) }
					className="presto-player__modal--style-name"
					// eslint-disable-next-line jsx-a11y/no-autofocus -- pre-existing focus behaviour for this modal; removing it changes focus management, out of scope for this translation PR.
					autoFocus
				/>
        ...
        audio when the controls are enabled.
				*/ }
				...
				<div className="presto-audio-presets-edit__footer">
					<div className="presto-audio-presets-edit__meta">
						Preset ID: { preset.id }
					</div>
					<div>
						<Button
