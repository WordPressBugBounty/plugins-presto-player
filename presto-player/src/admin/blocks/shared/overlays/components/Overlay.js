...
const Overlay = ( {
	...
	useEffect( () => {
		if ( timeToSeconds( startTime ) >= timeToSeconds( endTime ) ) {
			let nextEndTime = sanitizeTime( startTime );
			const seconds = timeToSeconds( nextEndTime ) + 1;
			nextEndTime = sanitizeTime( secondsToTime( seconds ) );
	...
	const updateStartTime = () => {
		const nextStartTime = sanitizeTime( draftStartTime );
		update( { startTime: nextStartTime } );
		setDraftStartTime( nextStartTime );
		updateCurrentTime( nextStartTime );
	};

	const updateEndTime = () => {
		const nextEndTime = sanitizeTime( draftEndTime );
		update( { endTime: nextEndTime } );
		setDraftEndTime( nextEndTime );
	};
		...
		<div className="presto-overlay">
			<Flex align="center" className={ className }>
				<FlexItem>
					<TextControl
						...
						onChange={ ( value ) => setDraftStartTime( value ) }
			...
			<BaseControl style={ { width: '100%' } }>
				...
				<UrlSelect
					...
					setSettings={ ( value ) => update( { link: value } ) }
					settings={ link || {} }
				/>
			</BaseControl>
