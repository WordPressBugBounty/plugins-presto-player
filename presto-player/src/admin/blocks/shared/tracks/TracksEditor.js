...
export default function TracksEditor( {
	 ...
	 * @return {boolean} True when the track comes from Bunny.net.
	 */
	 ...
	 *
	 * @param {Object} track The caption track to delete.
	 */
	const deleteBunnyNetCaption = async ( track ) => {
		...
		try {
			...
			if ( ! bunnyGuid ) {
				// eslint-disable-next-line no-console -- deliberate diagnostic for an unexpected missing Bunny.net GUID.
				console.warn(
			...
			if ( ! language ) {
				// eslint-disable-next-line no-console -- deliberate diagnostic for an undeterminable caption language.
				console.warn(
					'Could not determine language code for caption deletion'
				);
				return;
			}
			...
			// eslint-disable-next-line no-console -- deliberate error reporting for a failed API request.
	 ...
	 *
	 * @param {number} trackIndex Index of the track within the tracks array.
	 */
	...
	/**
	 * Get the transcribe modal content based on the current state.
	 *
	 * @return {Element} The modal body to render.
	 */
	const renderTranscribeModalContent = () => {
		if ( transcribeSuccess ) {
			return (
				<>
					<p>
