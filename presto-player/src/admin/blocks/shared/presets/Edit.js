		...
		try {
			...
			const saved = await wp.apiFetch( {
				...
				url: wp.url.addQueryArgs(
					`${ window.prestoPlayer.root }${ window.prestoPlayer.prestoVersionString }preset/${ preset.id }`,
					{ _method: 'PUT' }
				),
				data,
			} );
	...
	const createPreset = async () => {
		...
		try {
			const saved = await wp.apiFetch( {
				method: 'POST',
				url:
					window.prestoPlayer.root +
					window.prestoPlayer.prestoVersionString +
					'preset',
				data: {
					...{ name: thisName },
					...state,
	...
	return (
		...
		>
			<div className="presto-player__preset-options" data-cy="preset-modal">
				<TextControl
					...
					/* eslint-enable @wordpress/i18n-ellipsis */
        ...
        video when the controls are enabled.
				*/ }
				...
				<div className="presto-presets-edit__footer">
					<div className="presto-presets-edit__meta">
						Preset ID: { preset.id }
					</div>
					<div>
						<Button
