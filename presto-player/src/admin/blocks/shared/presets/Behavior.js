/* eslint-disable camelcase -- These identifiers (auto_hide, show_time_elapsed, save_player_position, etc.) are Presto Player preset field names stored in the DB and sent over REST; renaming them breaks the data contract. */
...
export default function ( { state, updateState, className } ) {
	return (
        ...
        />
			...
			{ !! state.sticky_scroll && (
				<>
					{ /* eslint-disable-next-line @wordpress/no-base-control-with-label-without-id -- adding an id would change the rendered markup/label association; out of scope for this translation-only change. */ }
					<BaseControl
						label={ __(
