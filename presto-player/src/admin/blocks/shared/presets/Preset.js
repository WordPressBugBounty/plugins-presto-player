...
export default function ( {
	...
	const trashPreset = async () => {
		...
		try {
			setLoading( true );
			const deleted = await wp.apiFetch( {
				method: 'POST',
			...
			// eslint-disable-next-line no-console -- deliberate error reporting: preset deletion failures are logged for debugging alongside the user-facing notice.
			console.error( e );
			if ( e?.message ) {
				wp.data.dispatch( 'core/notices' ).createNotice(
	...
	return (
		// eslint-disable-next-line jsx-a11y/click-events-have-key-events -- pre-existing markup; adding keyboard handling changes interaction behaviour and is out of scope for this translation PR.
		<div
		...
		>
			...
			{ ! preset?.is_locked && (
				<div className="block-editor-block-styles__item-edit">
					{ /* eslint-disable-next-line jsx-a11y/click-events-have-key-events, jsx-a11y/no-static-element-interactions -- pre-existing markup; converting to a button changes styling and keyboard behaviour, out of scope for this translation PR. */ }
					<div
