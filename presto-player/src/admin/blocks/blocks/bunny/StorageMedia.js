...
export default ( { closePopup, noticeOperations, onSelect, isPrivate } ) => {
	...
	const storeFile = async ( { path, name } ) => {
		...
		try {
			const storedVideos = await wp.apiFetch( {
			...
			setError( e.message );
			// eslint-disable-next-line no-console -- deliberate error reporting for upload failures, surfaced to developers alongside the UI notice.
			console.error( e );
		} finally {
			setProgress( 0 );
			setFetching( false );
	...
	const loadVideos = async () => {
		try {
			setFetching( true );
			const loadedVideos = await wp.apiFetch( {
				path: `presto-player/v1/bunny/${ endpoint }`,
			} );
