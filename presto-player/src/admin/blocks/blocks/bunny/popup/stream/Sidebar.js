...
export default () => {
	...
	const getThumbnail = ( item ) => {
		return isPrivate ? item?.webPURLSigned : item?.webPURL;
	};
			...
			<div className="presto-player__media-modal-sidebar-content">
					...
					<Disabled key={ video.id }>
						{ isSelectable( video ) && getThumbnail( video ) && (
							<img
								...
								alt=""
								className="presto-stream-sidebar__preview-img"
							/>
						) }
					</Disabled>
					<Button
						...
						className={ `presto-stream-sidebar__badge ${
							isSelectable( video ) ? 'is-selectable' : ''
						}` }
					>
						{ getStatusText( video ) }
					</Button>
				...
				{ !! video?.visibility && (
					<BaseControl>
						...
						<h3 className="presto-stream-sidebar__control">
							{ video.visibility }
						</h3>
					</BaseControl>
				) }
