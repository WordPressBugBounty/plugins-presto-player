...
export default () =>
	createHigherOrderComponent(
		( WrappedComponent ) => ( props ) => {
			...
			const { poster, chapters, ratio } = attributes;
			...
			useEffect( () => {
				const times = chapters?.map( ( item ) => item.time );
				const prevTimes = prevChapters?.map( ( item ) => item.time );
			...
			const createVideo = async ( {
				...
				// eslint-disable-next-line camelcase -- external_id/attachment_id are REST payload keys for the presto-player/v1/videos endpoint.
			...
			function onRemoveSrc() {
				// eslint-disable-next-line no-alert -- native confirm is the existing admin UX; replacing it with a modal is out of scope for this PR.
				const r = confirm( __( 'Remove this video?', 'presto-player' ) );
				if ( r ) {
					setAttributes( {
						src: '',
						id: undefined,
