...
const { BaseControl, ToggleControl, Notice } = wp.components;
...
export default ( {
	...
	return (
		<MediaPlaceholder
			...
			onSelect={ onSelect }
			value={ attributes }
			// Audio accepts a URL even when private; the video placeholder gates this
			// on ! isPrivate. MediaPlaceholder takes no onError prop, so none is passed.
			allowURLs={ true }
		>
			<div style={ { width: '100%' } }>
				<BaseControl className="presto-player__placeholder-control">
					<ToggleControl
						...
						onChange={ ( nextIsPrivate ) => {
							...
							setAttributes( {
								visibility: nextIsPrivate ? 'private' : 'public',
							} );
						} }
					/>
				</BaseControl>
