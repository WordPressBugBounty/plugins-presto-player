	...
	const quickAccessItems = [
		{
			...
			label: __( 'Explore AI Abilities', 'presto-player' ),
			badge: __( 'New', 'presto-player' ),
			link: `${
				prestoPlayerData?.dashboardUrl || '#'
			}&tab=Settings&section=mcp`,
			internal: true,
			// The MCP section reads /wp/v2/settings, so only show the shortcut to
			// users who can actually load it.
			condition:
				prestoPlayerData?.abilitiesSupported !== false &&
				prestoPlayerData?.canManageOptions !== false,
		},
		{
			id: '1',
						...
						button?.condition !== false && (
							...
							>
								...
								>
									...
									<Container.Item className="flex items-start gap-1.5">
										<Text className="px-1 py-0 text-sm font-medium text-text-primary">
											{ button.label }
										</Text>
										{ button.badge && (
											<span className="mt-0.5 shrink-0 rounded-full bg-brand-primary-600 px-1 text-[9px] font-semibold uppercase tracking-wide text-white">
												{ button.badge }
											</span>
										) }
									</Container.Item>
