...
const getNavItems = () => {
	...
	return NAV_ITEMS_ALL.filter( ( item ) => {
		if ( item.requireManageOptions && ! canManageEmails ) {
			return false;
		}
		return true;
	} );
...
const Navbar = () => {
	...
	const VERSION = window?.prestoPlayer?.version
		? sprintf(
				/* translators: %s: Presto Player version number */
				__( 'v%s', 'presto-player' ),
				window.prestoPlayer.version
		  )
		: false;

	const PRO_VERSION = window?.prestoPlayer?.proVersion
		? sprintf(
				/* translators: %s: Presto Player Pro version number */
				__( 'v%s', 'presto-player' ),
				window.prestoPlayer.proVersion
		  )
		: false;
					...
					<DropdownMenu.ContentWrapper>
						<DropdownMenu.Content className="w-60">
							<DropdownMenu.List>
								<DropdownMenu.Item
									onClick={ () =>
										window.open( 'https://prestoplayer.com/docs/' )
									}
								>
